...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Confirm button opens the fullscreen video overlay
  ...
  const continueVideoIframe = document.getElementById("continueVideoIframe");
  ...
  function openFullscreenVideo() {
    if (continueVideoIframe) {
      continueVideoIframe.src = continueVideoIframe.dataset.src || "";
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (slider && bgMusic) {
    ...
    // Use pointerdown for broad device support; touchstart as a fallback
...
// Enable background music playback on mobile by starting it after
// the first user gesture (touch/click). Mobile browsers require
// a user interaction to allow unmuted audio.
(function enableBgOnFirstInteraction(){
  ...
  function startBg() {
    try {
      bg.muted = false;
      bg.play().catch(err => console.log('bgMusic play rejected on gesture:', err));
    } finally {
  ...
  window.addEventListener('touchstart', startBg, {passive: true});
  window.addEventListener('click', startBg, {passive: true});
})();